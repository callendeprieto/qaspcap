pro 	ghconv,x,y,fwhm,par,nx,ny,ppr=ppr,nsigma=nsigma,inter=inter,original=original

;+
;	Convolution with a Gauss-Hermite kernel
;
;	IN:	x	-fltarr		independent variable
;		y	-fltarr		array of data to smooth 
;		fwhm-float      fwhm of the Gaussian
;		par -flotarr	other parameters of the kernel (pixels) [h3,h4]
;						(h0=1.0, h1=h2=0.0) 
;						h3 controls the asymmetry
;						h4 controls the peakiness (h4>0 more peaky than Gauss)
;	
...
;
;		IDL> fwhm=2.0/0.1
;		IDL> ghconv,w,f,fwhm,[0.0,0.0],w2,f2
;
...
;		IDL> ghconv,alog(w),f,fwhm,[0.0,0.0],w2,f2,/inter
;
...
;					 , IAC, August 2010 -- adapted from gconv.pro
;					 , IAC, July 2015 -- bug fixed (checking linear or log x scale)	
;  					        and changed ppr type from integer to float
;				
;-

if (N_params() lt 4) then begin
	message,'use -- ghconv,x,y,fwhm,[h3,h4],nx,ny[,ppr=ppr,nsigma=nsigma',/cont
...
;psf2=1.d0/sqrt(2.d0*!dpi)/sigma*exp(-((dindgen(2*grange+1)-grange)/sigma)^2/2.d0)
;psf2=psf2/total(psf2)
pars=[1.d0,0.0d0,sigma,1.0d0,0.0d0,0.0d0,par]
psf=gausshermite(dindgen(2*grange+1)-grange,pars)
psf=reverse(psf)/total(psf)
...
if max(wtrim) le 0 then begin
	message,'The Kernel is too wide for the input range',/cont
	return
endif
